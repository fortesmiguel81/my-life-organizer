...
import { sql } from 'drizzle-orm/sql'; // For custom SQL operations
  ...
  const data = await db
    ...
    .where(
      and(
        ...
        auth?.orgId ? eq(accounts.orgId, auth.orgId) : eq(accounts.userId, auth.userId) // Filter by orgId or userId from the accounts table
      )
    )
